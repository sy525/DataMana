...
/* Research Question
1. Does the majority elderly have a better health status than their minority counterparts, as the literature showed? 
2. Does the elderly living in the urban area live longer than their counterparts in the rural area?
*/
...
Hypothesis 4: The minority elderly living in an urban area have a better health status than those in a rural area.
*/
...
health observed among ethnic groups in different regions (rural/ urban). Based on Shaver’s model, 
...
the health status of the Chinese elderly. These control variables are variables: gender, marital status, family members.  
*/

/* Literature Review
Geographic health disparity is widespread in China, especially evident in the older population. A
lot of interesting studies have demonstrated that older people from urban areas can access more
...
inequality between these two regions (Grossman, 1972; Rodgers, 1979; Wilkinson, 1996;Subramanian &amp; Kawachi, 2004; Feng &amp; Yu, 2007). It is known that both income growth and
...
ethnic minority groups just account for 8.49% (NBS, 2010). The ethnic minorities are different from the Han majority on culture, customs, genetic backgrounds, geographic location and
...
*2. Tung, Ho-Jui, and Elizabeth J. Mutran. "Ethnicity and Health Disparities Among the Elderly in Taiwan." Research ///
...
* flip the health variable, make 5 mean the best outcome of health, 1 the worst outcome. 
recode health14 (5=1) (4=2)(3=3)(2=4) (1=5)(8 9 .=.),gen(rev_health14) 
la de healthlab 1 "very bad" 2 "bad" 3 "so so" 4 "good" 5 "very good"
la val rev_health14 healthlab
...
merge 1:1 ID using eldfinal,nogen
...
/*****************/
/* Sulman
The variable you can play with are: dependent (rev_health14 function14) ; independent ( income14 education14 employment14 ); 
...
twoway (bar rev_health14 rural14, sort), by(rural14, total) subtitle(, size(medium))
...
graph bar (mean) rev_health14, over(major14) blabel(bar) by(rural14, total)
...
graph hbar (count), over(sex14) nofill cw blabel(total) by(rev_health14, total)
...
symplot income14
gr export graph19.png, replace  

symplot marital14
gr export graph20.png, replace  
* Bin what's this?
...
graph matrix rev_health14 income14, by(rural14, total)
gr export graph142.png, replace  

qnorm age14, grid
gr export graph23.png, replace  
* Bin what's this?
...
reg rev_health14 rural14 ethnic14  income14 
reg rev_health14 rural14 ethnic14  income14 education14 employment14
reg rev_health14 rural14 ethnic14  `varin'
reg rev_health14 rural14 ethnic14  `varin' 
...
// Robust Check
reg rev_health14 income14 , robust
scatter rev_health14 income14 ,ml(ethnic14)
...
reg rev_health14 Urban14 ethnic14 income14 education14 employment14 smoking14 marital14 familysize14 sex14
rvfplot, yline(0)
...
/* *then i run some otjer specification */
reg rev_health14 rural14 ethnic14 income14 education14 
...
reg rev_health14 rural14 ethnic14 income14 education14 employment14 smoking14
/* *and outreg2 again but i append instead of replace */
outreg2 using reg5.xls, onecol bdec(2) st(coef) excel append ct(A6) lab  
reg rev_health14 rural14 ethnic14 income14 education14 employment14 smoking14 marital14
/* *and outreg2 again but i append instead of replace */
outreg2 using reg5.xls, onecol bdec(2) st(coef) excel append ct(A7) lab
reg rev_health14 rural14 ethnic14 income14 education14 employment14 smoking14 marital14 familysize14
/* *and outreg2 again but i append instead of replace */
outreg2 using reg5.xls, onecol bdec(2) st(coef) excel append ct(A8) lab
reg rev_health14 rural14 ethnic14 income14 education14 employment14 smoking14 marital14 familysize14 sex14
/* *and outreg2 again but i append instead of replace */
