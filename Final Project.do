...
counterparts. Some claim that the unequal distribution of health resources is 
a major contributor. Health resources, such as community health centers, health 
facilities, nursing homes, professional doctors are concentrated in urban settings. 
Numerous health programs for elderly which are funded by local governments are 
located in urban areas. Generally, the urban elderly receive high-quality health 
service. In contrast, older people in rural area confront many barriers to access 
to health service. One of the barriers is that the caregivers are unwilling to 
work in rural areas due to the impoverished living conditions. Other barriers are worsening 
income inequality between these two regions. It is known that both income growth 
and income inequality influence an individual’s health condition . The income per 
person in the urban region is higher than that of the rural region. According to the 
National Bureau of the statistic of China, per capita disposable income in rural
households is $1945 while $5272 in the urban household in 2017. With
...
care service because of the financial issue. Many of them are more 
likely to receive self-treatment rather than inpatient service than urban 
counterparts. China has 56 official ethnic groups—55 ethnic minority 
...
account for 8.49%. The ethnic minorities are different from  the Han 
majority on culture, customs, genetic backgrounds, geographic location and 
languages. Because of historical reasons, the ethnic minority 
communities are more concentrated in the mountain area where economic development 
is hindered. Plus, to support the ethnic minority communities, the Chinese government 
has set up five minority autonomous regions, which has the highest minority ethnic population. 
...
regional economic growth in China. Constraint by finance, ethnic minorities 
that tend to live in the less developed geographic region has poorer health status than
the Han majority.
*/ 
...
an older individual’s ability status in the daily life. 
...
status than urban elderly, which is very interesting. We looked deeper on the 
...
because of the small sample size. Consequently, our findings 
...
disparity. Simplification of the health disparity cannot 
...
Among the Elderly in Taiwan." Research on Aging27, no.3(2005): 327-354.
...
/*
The reasons for the non-merge is that there are thousands of new observations 
added to the using datasets. Also, a lot of observations that is in the master 
dataset are not exist in using dataset becasue the researchers cannot track these
respondents any more. 
*/
*/
...
twoway(scatter rev_health14 age14) if age14>65|| lfit rev_health14 age14, ///
title(Age and Health Assocation Over 65)
...
* use matrix to visualize the correlation between variables. It enables us to observe the pattern.
...
///the main determinants of elderly's health
...
xi: regress rev_health14 i.rural14 ethnic14 income14 education14 employment14 ///
happiness14 smoking14 marital14 familysize14 sex14 age14,robust
...
xi: regress rev_health14 i.ethnic14 Urban14 logincome education14 employment14 ///
...
/* 
We expected to see the urban/rural health disparity among the elderly. Here, we use income, 
education and  employment to measure health disparity. Using interaction between these variables, 
we can find that interaction effect of urban employment on health is significant, 
controled for other variables. In other words, ethnic health disparity do exist among elder population. 
It support our hypothese that majority elderly have better health than minorities elderly.  
*/
...
estat imtest
estat szroeter, rhs
/*we can find that there is heteroskedascity in the regression model. Therefore, some
approaches need to use to minimize the heteroskedascity, like adding "robust" to regression.
*/ 
...
reg rev_health14 rural14 ethnic14 logincome education14 employment14 smoking14 ///
marital14 familysize14 sex14 age14
